...
//            $jobs['total'] = $totaljobs;
    ...
    public function store($cuser, $data)
    {
        ...
        if ($user->user_type ==== env('CUSTOMER_ROLE_ID')) {
            ...
            $data['job_for'] = [];
            if ($job->gender) {
                $data['job_for'][] = $this->getJobGender($job->gender);
            }
            if ($job->certified) {
    ...
    public function jobEnd($post_data = [])
    {
        ...
        $email = $this->getEmailAddress($user, $job);
        ...
        $this->sendJobEndEmail($email, $user, $tr, $name, $job, 'faktura');
    }
    ...
    public function getPotentialJobIdsWithUserId($user_id)
    {
        ...
        $job_ids = Job::getJobs($user_id, $job_type, 'pending', $userlanguage, $gender, $translator_level)->filter(function($job_id) use ($user_id){
...
//        if (in_array($data['status'], ['pending', 'assigned']) && date('Y-m-d H:i:s') <= $job->due) {
        ...
        if ($data['status'] === 'pending') {
            ...
            $job->save();
            $this->sendJobAcceptedEmail($email, $name, $dataEmail);
            return true;
        }
...
//        if (in_array($data['status'], ['withdrawnbefore24', 'withdrawafter24', 'timedout', 'assigned'])) {
    ...
    {
        $user = $job->user()->first();
        $email = $this->getEmailAddress($use, $job);
        $name = $user->name;
    ...
    public function sendChangedDateNotification($job, $old_time)
    {
        ...
        $this->sendChangedDateEmail($email,$user, $translator, $data);
    }
    ...
    public function sendChangedLangNotification($job, $old_lang)
    {
        ...
        $this->sendChangedLangEmail($email, $name, $translator, $data);
    }
    ...
    public function sendNotificationByAdminCancelJob($job_id)
    {
        ...
        if ($job->certified) {
            $this->getJobCertification($job->certified);
        }
    ...
    public function acceptJob($data, $user)
    {
        ...
        if (!Job::isTranslatorAlreadyBooked($job_id, $user->id, $job->due)) {
            if ($job->status === 'pending' && Job::insertTranslatorJobRel($user->id, $job_id)) {
                ...
                $this->sendJobAcceptedEmail($email, $name, $data);
            }
            /*@todo
...
//        $this->logger->addInfo('USER #' . Auth::user()->id . ' reopen booking #: ' . $jobid);
    ...
    private function getJobGender($gender)
    {
     ...
     * @param @for_text
     */
    private function sendJobEndEmail($email, $user, $translator, $name, $job, $for_text)
    {
        $subject = 'Information om avslutad tolkning för bokningsnummer # ' . $job->id;
        // $session_explode = explode(':', $job->session_time);
        // $session_time = $session_explode[0] . ' tim ' . $session_explode[1] . ' min';
        $session_time = $this->getSessionTime($job->session_time);
        $data = [
            'user'         => $user,
            'job'          => $job,
     ...
     * @param $dataEmail
     */
    private function sendJobAcceptedEmail($email, $name, $dataEmail)
    {
        $subject = 'Bekräftelse - tolk har accepterat er bokning (bokning # ' . $job->id . ')';
        $this->mailer->send($email, $name, $subject, 'emails.job-accepted', $dataEmail);
    }
     ...
     * @return string
     */
    private function getSessionTime($session_time)
    {
     ...
     * @return string
     */
    private function getEmailAddress($user, $job)
    {
     ...
     * @param $data
     */
    private function sendChangedDateEmail($email, $user, $translator, $data)
    {
        $subject = 'Meddelande om ändring av tolkbokning för uppdrag # ' . $job->id . '';
     ...
     * @param $data
     */
    private function sendChangedLangEmail($email, $name, $translator, $data)
    {
     ...
     * @return array
     */
    private function getJobCertification($certification)
    {
        $return  = [];
        if ($certification === 'both'){
            $return[] = 'normal';
            $return[] = 'certified';
        }elseif ($certification === 'yes'){
            $return[] = 'certified';
        }else {
